...
#    gray = cv2.cvtColor(images, cv2.COLOR_BGR2GRAY)
 ...
 #   print(images)
   # rshape = images.reshape(-1, 720, 480, 3)
...
def load_cached(cache_path, in_dir):
    ...
    return dataset
def random_batch(images,batchSize,labels):
    ...
    x_batch = np.asarray(images)[idx]
    y_batch = np.asarray(labels)[idx]
